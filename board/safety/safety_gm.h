...
static int gm_tx_hook(CANPacket_t *to_send) {
  ...
  if ((addr == 481) && (gm_pcm_cruise || gm_hw == GM_CAM_CC)) {
    ...
    allowed_btn |= cruise_engaged_prev && (gm_hw == GM_CAM_CC) && (button == GM_BTN_SET || button == GM_BTN_RESUME || button == GM_BTN_UNPRESS);
    if (!allowed_btn) {
      tx = 0;
    }
  }
...
#endif
  gm_pcm_cruise = ((gm_hw == GM_CAM) && !gm_cam_long) || ((gm_hw == GM_CAM_CC) && !gas_interceptor_detected);
  return &gm_rx_checks;
}
